...
for(let i = 0; i < flatCellList.length; i++){
    ...
    if (r < boardSizeForCells && c < boardSizeForCells) {
        if (!boardCellElements[r]) boardCellElements[r] = [];
...
function renderBoard() {
    ...
    for(let r = 0; r < size; r++)for(let c = 0; c < size; c++){
        ...
        if (previousValue !== 0) cellElement.classList.remove(`field-cell--${previousValue}`);
        if (value !== 0) cellElement.classList.add(`field-cell--${value}`);
...
class Game {
    ...
    _move(direction) {
        ...
        let currentBoard = this.board.map((row)=>[
                ...row
            ]);
        if (direction === "up" || direction === "down") currentBoard = this._transpose(currentBoard);
        if (direction === "right" || direction === "down") currentBoard = currentBoard.map((row)=>[
                ...row
            ].reverse());
        const newBoard = [];
        ...
        if (direction === "up" || direction === "down") currentBoard = this._transpose(currentBoard);
        boardChanged = this.board.map((row)=>row.join(",")).join("|") !== currentBoard.map((row)=>row.join(",")).join("|");
    ...
    _processRowLeftInternal(inputRow) {
        ...
        while(shiftedRow.length < this.size)shiftedRow.push(0);
        ...
        while(finalProcessedRow.length < this.size)finalProcessedRow.push(0);
    ...
    _transpose(boardToTranspose) {
        const newBoard = Array.from({
            ...
            Array(this.size).fill(0));
        for(let r = 0; r < this.size; r++)for(let c = 0; c < this.size; c++)newBoard[c][r] = boardToTranspose[r][c];
        return newBoard;
    }
    _addRandomTile() {
        const emptyCells = [];
        for(let rowIdx = 0; rowIdx < this.size; rowIdx++){
            for(let colIdx = 0; colIdx < this.size; colIdx++)if (this.board[rowIdx][colIdx] === 0) emptyCells.push([
                rowIdx,
                colIdx
            ]);
        }
    ...
    _canMove() {
        ...
        for(let r2 = 0; r2 < this.size; r2++)for(let c2 = 0; c2 < this.size; c2++){
            const current = this.board[r2][c2];
            if (c2 + 1 < this.size && current === this.board[r2][c2 + 1]) return true;
            if (r2 + 1 < this.size && current === this.board[r2 + 1][c2]) return true;
        }
        return false;
    }
    _checkGameStatus() {
        if (this.status !== "playing") return;
        if (this._checkForWin()) this.status = "win";
        else if (!this._canMove()) this.status = "lose";
    }
}
